#THIS FILE GETS THE FACE IMAGES AND PROCESSES THEM APPROPRIATELY
#FOR PART 8 - 10
...
def main(gender):
    ...
    dirs = ["uncropped", "cropped", "cropped_color"]
    for direct in dirs:
    ...
    for a in ACTORS:
        ...
        for line in open(SRC_FILE):
            if a in line: 
                ...
                if (ret != None) and ret:
                    if sha != hashlib.sha256(open("uncropped/"+filename, "rb").read()).hexdigest():
                    ...
                    try:
                        ...
                        final_im = imresize(gray_scale_im, size=(64,64,3)) 
                        im = toimage(final_im)
                        im.save("cropped/"+filename)

                        #Colored pictures
                        final_im_col = imresize(face/255., size=(64,64,3))
                        im_col = toimage(final_im_col) 
                        im_col.save("cropped_color/"+filename)
                        i += 1
                    except:
...
if __name__ == "__main__":
    ...
    #Gets the images for both female and male.
    #main("female")
    #main("male")
